...
contract LurePlan {
    ...
    string public constant MSG_AMOUNT_EXEEDS_BALANCE =
        "Actually the Smart Contract doesn't have this amount to retire now. Try Later or Contact Support. Thanks.";
    ...
    constructor(
        uint256 _termDays,
        uint256 _erc20_min_deposit,
        uint256 _erc20_max_deposit,
        uint256 _bnb_min_deposit,
    ...
    function withdraw() public payable {
        ...
        require(
            block.timestamp >= investment_ends_of[msg.sender],
            MSG_PLAN_ENDS
        );

        // Transferimos la cantidad al usuario
        address wallet = msg.sender;
        address token = erc20_of_plan[msg.sender];
        uint256 amount = get_amount_to_claim_with_taxes(msg.sender);

        if (token == address(0)) {
            require(address(this).balance >= amount, MSG_AMOUNT_EXEEDS_BALANCE);
            ...
            require(
                ...
                MSG_AMOUNT_EXEEDS_BALANCE
            );
            require(erc20.transfer(wallet, amount), MSG_WITHDRAW_FAIL);
        }
    ...
    function get_amount_to_claim(address wallet) public view returns (uint256) {
        return
            investment_of[wallet] +
            ((investment_of[wallet] * total_interest) / 100);
    }
    ...
    function get_amount_to_claim_with_taxes(
        address wallet
    ) public view returns (uint256) {
        return
            get_amount_to_claim(wallet) -
            get_taxes(5, get_amount_to_claim(wallet));
    }
